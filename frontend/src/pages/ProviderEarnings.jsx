...
export default function ProviderEarnings() {
  ...
  useEffect(() => {
    getMyEarnings().then(r => setData(r.data)).finally(() => setLoading(false))
  }, [])
  ...
  const monthDiff = (this_month || 0) - (last_month || 0)
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#f8fafc', fontFamily:'system-ui,-apple-system,sans-serif' }}>
      ...
      {/* Header — same gradient + padding as all other pages */}
      <div style={{ background:'linear-gradient(135deg,#7c3aed 0%,#4338ca 100%)', padding:'40px 40px 36px' }}>
        <div style={{ maxWidth:'900px', margin:'0 auto' }}>
          <Link to="/provider"
            style={{ display:'inline-flex', alignItems:'center', gap:'8px', color:'rgba(255,255,255,0.7)', fontSize:'14px', fontWeight:'600', textDecoration:'none', marginBottom:'20px' }}
            onMouseOver={e => e.currentTarget.style.color='white'}
            onMouseOut={e => e.currentTarget.style.color='rgba(255,255,255,0.7)'}>
            <ArrowLeft size={15}/> Provider Dashboard
          </Link>
          <h1 style={{ fontSize:'clamp(22px,3vw,32px)', fontWeight:'900', color:'white', marginBottom:'6px', letterSpacing:'-0.4px' }}>Earnings</h1>
          <p style={{ color:'rgba(255,255,255,0.7)', fontSize:'15px' }}>Your completed-job revenue summary</p>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'900px', margin:'0 auto', padding:'32px 40px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'16px', marginBottom:'28px' }}>
          {[
            { label:'Total Earned',  value: total_earned || 0,  sub: `${total_jobs || 0} completed jobs`, color:'#7c3aed' },
            { label:'This Month',    value: this_month   || 0,  sub: monthPct !== null ? null : 'No prior month data', pct: monthPct, color:'#059669' },
            { label:'Last Month',    value: last_month   || 0,  sub: 'Previous month total', color:'#0ea5e9' },
          ].map(({ label, value, sub, pct, color }, i) => (
            <motion.div key={label} initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} transition={{duration:0.3, delay:i*0.07}}
              ...
              <div style={{ display:'flex', alignItems:'center', gap:'3px', fontSize:'24px', fontWeight:'900', color:'#0f172a', marginBottom:'6px' }}>
                <IndianRupee size={18} strokeWidth={2.5}/>
                {parseFloat(value).toLocaleString('en-IN')}
              </div>
              <div style={{ fontSize:'12px', color:'#64748b' }}>
                {pct !== undefined && pct !== null ? <TrendBadge pct={pct}/> : sub}
              </div>
            </motion.div>
          ))}
        </div>
              ...
              {recent.map((b, i) => (
                <motion.div key={b.id} initial={{opacity:0,x:-8}} animate={{opacity:1,x:0}} transition={{duration:0.25, delay: i * 0.04}}
                  ...
                  <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
                    <div style={{ width:'36px', height:'36px', borderRadius:'10px', background:'#f5f3ff', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'16px', flexShrink:0 }}>
                      {b.service_icon || '🔧'}
                    </div>
                    <div>
...
function TrendBadge({ pct }) {
  if (pct === 0) return (
    <span style={{ display:'inline-flex', alignItems:'center', gap:'4px', fontSize:'12px', color:'#64748b' }}>
      <Minus size={12}/> Same as last month
    </span>
  )
  const up = pct > 0
  return (
